...
addEventListener('message', ({ data }) => {
  ...
  if (!['TON', 'KOD'].includes((d[1][0] + '').toUpperCase())) {
    errors.push(`A2 (${d[1][0]}) - 'TON' yoki 'KOD' bo'lishi shart`);
  }
  ...
  // let i = 1;
  // while (i < 1000) Math.pow(i, 3) * Math.sin(i);

  postMessage(errors);
});

function numberToMark(num: number) {
  if (1 <= num && num <= 26) {
